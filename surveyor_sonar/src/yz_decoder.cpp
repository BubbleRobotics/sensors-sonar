...
constexpr uint16_t kYZPointDataMessageId = 3011;
constexpr uint16_t kATOFPointDataMessageId = 3012;
constexpr size_t kYZMinimumPayloadSize = 100;
             ...
             std::vector<YZPoint>& points_out)
{
    ...
    if (msg_id == kYZPointDataMessageId)
    {
        if (payload_len < kYZMinimumPayloadSize)
        {
            return false;
        }

        size_t offset = 0;
        offset += 4;   // timestamp_msec
        offset += 4;   // ping_number
        offset += 4;   // sos_mps
        offset += 12;  // up_vec
        offset += 12;  // mag_vec
        offset += 40;  // reserved[10]
        offset += 4;   // water_degC
        offset += 4;   // water_bar
        offset += 4;   // heave_m
        offset += 4;   // start_m
        ...
        const size_t points_bytes = static_cast<size_t>(num_points) * 2U * sizeof(float);
        ...
        {
            const float y = readFloatLE(payload + offset);
            offset += 4;
            const float z = readFloatLE(payload + offset);
            offset += 4;
